...
| 项目 | 基线 |
|---|---|
| 最后验证 | `2026-07-17`；已对照 AMD、Microsoft、Canonical、ONNX Runtime、Docker Hub 与 PyPI 一手资料 |
| 主机 | Windows、Ubuntu；支持门槛取决于 GPU/NPU 代际 |
| 路线 | DirectML、Windows ML MIGraphX、ROCm/MIGraphX、Ryzen AI/Vitis AI |
| 验证入口 | [`provider_test.py`](provider_test.py) |
| 证明范围 | 本次节点放置与输出基本有效性；内嵌 GPU 模型或带 `--compare-cpu` 的自定义模型还会验证 CPU 数值一致性 |
| 验证边界 | 脚本自检已在 Linux 通过；DirectML、Windows ML、MIGraphX、Vitis AI 最终证明仍需匹配硬件 |
...
| Ubuntu，列于对应 ONNX 矩阵的 AMD GPU | ROCm + MIGraphX + AMD wheel | `MIGraphXExecutionProvider` | **Linux GPU 的主路径**；必须满足精确 GPU、ROCm、Python 与 wheel 门槛，旧 `ROCMExecutionProvider` 已淘汰 |
...
| Windows 原生 ROCm Core SDK | **当前不能作为 ORT MIGraphX Python 路径** | 无 | ROCm 7.14 已扩展 Windows core 支持，但 AMD 当前验证的 MIGraphX/ORT 栈仅支持 Linux；应使用 DirectML 或 Windows ML |
...
| ROCm 7.14 已验证 ONNX 栈 | ORT 1.23.2 + MIGraphX 2.16 | AMD 当前仅支持 Linux、Python 3.12 与 `gfx950`/`gfx942`；不能把它直接套用到下面覆盖面更广的 7.2.x 路线 |
| 已审计 AMD 官方 MIGraphX wheel 路线 | ROCm 7.2.4 + ORT 1.23.2 | AMD 发行版匹配目录提供 CPython 3.10/3.12 wheel；它仍是验证器保留的较广泛可复现路线 |
...
| 最新 upstream ONNX Runtime / PyPI MIGraphX 包 | 1.27.1 | Upstream 与 PyPI wheel 于 2026-07-12 发布；AMD 尚未给出匹配的广泛 ROCm 兼容矩阵或 `repo.radeon.com` 发行目录，因此本指南不会把它替换进已审计路线 |
...
| Python 打包依赖 | pip 26.1.2；NumPy 最新为 2.5.1 | 本指南使用当前 pip，但因 AMD 明确记录 Radeon 7.2.1 ORT wheel 与 NumPy 2.x 不兼容，仍有意固定 NumPy 1.26.4；NumPy 2.5.1 也不支持 Python 3.10 路线 |
...
> **版本规则：** “最新”不等于兼容。ROCm、MIGraphX 和 ORT MIGraphX wheel 必须来自厂商验证的同一发行组合。Windows ML 的两个 `wasdk-*` 包与 Windows App Runtime 也必须属于同一发行线，且 machine-learning projection 的精确 ORT 依赖优先于独立 ORT 的最新版本。不要在同一虚拟环境中同时安装多个 `onnxruntime-*` 发行包。
...
验证器会强制下列 SHA-256。它们于 2026-07-17 从所述 Microsoft PyPI 或 AMD HTTPS 来源下载并重新计算。哈希不匹配时必须 fail closed，不能绕过检查；应重新审计新厂商 artifact，并同时更新代码和文档。
...
AMD 当前发布三条不同的 ONNX Runtime 路线；最新 ROCm Core SDK 不会自动成为每张 GPU 的正确 ONNX 包：

1. **当前 ROCm 7.14 路线：** ORT 1.23.2 + MIGraphX 2.16，Linux x86-64、Python 3.12，并且仅支持 `gfx950`（MI350X/MI355X）或 `gfx942`（MI300X/MI325X）。本指南的当前路线使用 Ubuntu 24.04，以便直接使用系统自带 Python 3.12。
2. **保留的 ROCm 7.2.4 路线：** AMD 仍提供旧版匹配的 ORT 1.23.2 CPython 3.10/3.12 wheel 与 Docker 镜像。它是已审计兼容路线，不是当前 ROCm 生产版本；部署前必须核对归档的 7.2.4 硬件矩阵与 AMD 支持策略。
3. **Radeon 专用 ROCm 7.2.1 路线：** AMD 当前 Radeon ONNX 矩阵仍在部分 Radeon 9000/7000 与 Radeon PRO 产品上验证 ORT 1.23.2，要求 Ubuntu 24.04.4 HWE 6.17 或 Ubuntu 22.04.5 HWE 6.8。

禁止混用不同路线的驱动、MIGraphX 包或 wheel。
...
| 其他 Instinct | MI300A、MI200 系列、MI100 | 当前 7.14 ONNX 行未列出这些 target；只能使用明确受支持的归档路线或单独验证的源码构建 |
...
**Ryzen APU iGPU 注意事项：** ROCm 7.14 已为多个 `gfx115x` Ryzen APU 增加 core GPU 支持，但当前 AI Ecosystem ONNX 行仍仅限 `gfx950/gfx942`；独立 Radeon 7.2.1 ONNX 矩阵也没有授予 Ryzen APU 生产支持行。不能从 core ROCm 成功枚举推断 ONNX 支持。Ryzen AI 笔记本在 Windows 上的广泛 GPU 路径是 DirectML；Ubuntu 上有文档支持的 STX/KRK 加速路径则是 Vitis AI NPU。

## 6. 安装匹配的 ROCm 路线

复制命令块前：

1. 在所选路线的 AMD 矩阵中确认精确 GPU SKU、LLVM target、OS point release 与 kernel。
2. 只能选择一条路线：
  - **当前 Instinct 路线：** 仅 `gfx950/gfx942`；使用 ROCm 7.14.0 与 6.1 节。
  - **保留的旧版 Instinct 路线：** 仅在归档矩阵与支持策略允许时使用 7.2.4；使用 6.2 或 6.3 节。
  - **Radeon 专用路线：** 仅适用于 AMD Radeon ONNX 矩阵明确列出的独立 Radeon/Radeon PRO；使用 7.2.1 和 6.4 节。
...
下列路线会安装或替换 GPU 软件，并可能要求重启。只能使用与精确硬件、发行版和 Ubuntu 版本匹配的路线。

### 6.1 当前 ROCm 7.14.0 ONNX 路线 — Ubuntu 24.04，仅 `gfx950/gfx942`

ROCm 7.14 使用新的 TheRock 打包方式。禁止改写下面旧版 `amdgpu-install_7.2.x` 命令。打开 AMD 当前 [ROCm 安装选择器](https://rocm.docs.amd.com/en/latest/install/rocm.html)，选择精确 GPU 与 Ubuntu 24.04，并完成其中的驱动和仓库前置步骤。使用 package-manager 路线时，注册 AMD 当前仓库后只能安装一个架构包：
...
### 6.2 保留的 ROCm 7.2.4 路线 — Ubuntu 24.04
...
### 6.3 保留的 ROCm 7.2.4 路线 — Ubuntu 22.04
...
### 6.4 Radeon ONNX 专用路线 — ROCm 7.2.1
...
**当前 ROCm 7.14** 路线必须安装 AMD 的精确 MIGraphX 2.16 包。已核查 runtime 包会提供 `/opt/rocm/bin/migraphx-driver`，并依赖 7.14 runtime：
...
本指南使用三条 AMD 匹配的 `onnxruntime_migraphx-1.23.2` 路线：ROCm 7.14.0 仅支持 CPython 3.12，并受 `gfx950/gfx942` 硬件门槛限制；保留的 ROCm 7.2.4 与 Radeon 7.2.1 仓库提供 CPython 3.10/3.12 wheel。应使用 Ubuntu 原生 Python：Ubuntu 24.04 使用 3.12；Ubuntu 22.04 的 7.2.x 路线使用 3.10。不要仅为本演示添加非官方 Python 仓库。
...
这里刻意使用 AMD wheel 直链：7.14 artifact 来自 `rocm.frameworks.amd.com`，7.2.x artifact 来自精确的 `repo.radeon.com` 发行目录。PyPI 已存在独立发布的同名 wheel，包括 1.27.1，但 AMD 尚未将它们映射到这些发行路线。`pip install ... -f` 已无法证明 pip 最终选择了哪个构建。演示脚本的 `--bootstrap` 路线会在安装前检查所选 AMD wheel 是否匹配 2026-07-17 审计记录的 SHA-256。

显式 PyPI index 可防止个人 pip mirror 设置静默改变依赖来源。在受管或离线环境中，只能使用已独立验证 artifact 与更新策略的组织镜像。

为什么固定 NumPy 1.26.4？AMD Radeon 7.2.1 ORT 页面明确警告其 wheel 与 NumPy 2.x 不兼容。本指南让三条 ORT 1.23.2 路线共用一个保守依赖基线。只有 AMD 发布更新的发行版匹配 ORT 栈并完成硬件验证后才应重新评估该固定版本。
...
截至 2026-07-17，AMD 尚未发布 ROCm 7.14 `rocm/onnxruntime` 镜像；最新官方标签仍为 7.2.4。因此此 Docker 快速路径仅适用于 7.2.x 路线；当前 7.14 路线应使用 6.1 和 7 节。
...
### 10.3 为什么 Windows 原生 ROCm 不是这条 ORT 路径
...
| `--bootstrap` | 要求干净隔离环境；固定并校验 Python 3.12 Microsoft DirectML wheel，或 AMD 官方直链 ROCm 7.2.1/7.2.4/7.14.0 wheel；强制 7.14 `gfx942/gfx950` 门槛，先暂存依赖，不安装驱动，也不卸载已有 ORT |
...
| Ubuntu AMD GPU | `python AMD/provider_test.py --target migraphx --bootstrap --strict-all`（自动检测 7.2.1、7.2.4 或带硬件门槛的 7.14.0 路线） |
