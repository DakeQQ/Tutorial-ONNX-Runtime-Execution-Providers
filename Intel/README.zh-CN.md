...
| 项目 | 基线 |
|---|---|
| 最后验证 | `2026-07-17`；已核对官方发布页与 PyPI 实际文件 |
| 主机 | Windows 11、Ubuntu x86-64 |
...
| 目标 | Intel CPU、集成/独立 GPU、集成 NPU 与显式 Meta-device |
| 验证入口 | `run_demo.bat`、`run_demo.sh`、[`provider_test.py`](provider_test.py) |
| 验证边界 | Ubuntu `CPU`、`GPU`、`GPU.0`、`GPU.1` 已通过真机测试；Windows 经静态审计；NPU 步骤经一手资料核验，仍需目标机器证明 |
...
截至 2026-07-17 审计，最新 Linux NPU 发布为 **v1.33.0**，验证组合是 OpenVINO 2026.2 与 Level Zero 1.27.0。它不能直接替代本文锁定教程中的 v1.28.0。
...
截至本次审计，`onnxruntime-openvino 1.24.1`（EP 5.9）仍是最新发布的 OpenVINO EP wheel，并明确配套 OpenVINO 2025.4.1。独立 `openvino` 项目已经更新到 2026.2.1，但 Intel 尚未发布与该运行时配套的 `onnxruntime-openvino`。因此，在本 EP 环境中替换成 `openvino==2026.2.1` 或运行 `pip install -U openvino` **不是**升级路线。应等待 Intel 发布明确给出新 ORT/OpenVINO 组合的新 EP。
...
以上是通过环境标记选择的顶层包精确锁定，并非带哈希、字节级锁定的供应链 lockfile。兼容的间接依赖仍按各包元数据解析，两个启动器都会在推理前运行 `pip check`。
...
**时效规则：** 应先检查 Intel 最新 EP 发布，再检查独立 OpenVINO 发布。更高的独立 OpenVINO 版本不会取代 EP 明确配套的版本。新版 `onnxruntime-openvino` 发布后，先读 Intel 发布说明，同时更新 ORT 与 OpenVINO 锁定值，检查 PyPI 实际文件名（不能只看分类器），再重新验证 CPU/GPU/NPU 驱动并运行严格显式设备测试。`onnx`、`numpy` 等辅助包只能在确认 Python/wheel 范围可解析且冒烟测试通过后单独更新。
