# ONNX Runtime NVIDIA GPU 完整指南：Windows 与 Ubuntu

> **最后核对：2026-07-16。** 可复现基线：ONNX Runtime 1.27.0、CUDA 13.0 Update 2 运行组件、cuDNN 9.14.0.64、传统 TensorRT 10.14.1.48，以及 TensorRT RTX EP ABI 独立插件 0.3.0。
>
> [English](README.md) · [项目首页](../README.md)

这是本仓库 NVIDIA 部分唯一的简体中文指南。从一台尚未配置 AI 环境的 NVIDIA 电脑开始，完整说明兼容规则、安装、严格执行证明、应用配置、性能选项、缓存、故障排查和升级，覆盖：

- `CUDAExecutionProvider`；
- 传统 `TensorrtExecutionProvider`；
- `nv_tensorrt_rtx` 独立 EP ABI 插件。
...
|---|---|---|---:|---|---:|
| CUDA EP | `onnxruntime-gpu==1.27.0` | 选定的 `cuda-toolkit==13.0.2` 组件 + `nvidia-cudnn-cu13==9.14.0.64` | 3.11–3.14 x64 | Turing，计算能力 7.5+ | 580+ |
| 传统 TensorRT EP | 同一 CUDA 核心 | 上述组件 + TensorRT **10.14.1.48** | 3.11–3.13 x64 | TensorRT 支持的 Turing+ | 580+ |
| TensorRT RTX 默认插件 | `onnxruntime==1.27.0` + 插件 `0.3.0` | CUDA 13 变体；内置 TensorRT RTX 1.5 运行库 | 3.11–3.14 x64 | Ampere 或更新 **RTX**，通常 GeForce RTX 30+ | 580+ |
...
本指南覆盖原生 Windows 10/11 x64 与 Ubuntu 22.04/24.04 x86-64。Jetson 必须使用 JetPack 对应的软件包，不属于本桌面教程范围。
...
ORT 1.27 元数据仍通过已迁移的 `nvidia-*-cu13` 发行包名引用多数 CUDA 13 组件。NVIDIA 后来把这些组件移动到无后缀发行包，并把旧名称保留为 `0.0.1` 迁移占位包，因此截至核对日期该 extra 已无法解析。本仓库改用 NVIDIA 当前的 `cuda-toolkit==13.0.2` 组件元包。
...
- ORT 1.27 普通 PyPI GPU wheel 使用 CUDA 13.0 与 cuDNN 9.14.0.64 构建；发布说明已将 CUDA 12 标记为弃用。
- `cuda-toolkit==13.0.2` 按 NVIDIA 当前包名提供一致的 CUDA 13.0 Update 2 组件集。
- ORT 1.27 传统 provider 构建使用 TensorRT 10.14.1.48。当前不固定版本的 `tensorrt-cu13` 是 TensorRT 11，不能直接替代 `nvinfer` 主版本 10。
...
| 包 | 固定版本 | 用途 |
|---|---:|---|
| `onnxruntime-gpu` | `1.27.0` | ORT CUDA 13 二进制及内置 CUDA/传统 TensorRT provider |
| 选定的 `cuda-toolkit` extra | `13.0.2` | cuBLAS、CUDA runtime、cuFFT、cuRAND、nvJitLink、NVRTC |
| `nvidia-cudnn-cu13` | `9.14.0.64` | 匹配的 cuDNN 运行库 |
...
Windows 从 [CUDA Toolkit Archive](https://developer.nvidia.com/cuda-toolkit-archive)下载 CUDA 13.x；安装器提供其他驱动时保留较新的正常驱动；只在需要时安装匹配 cuDNN 9；新开终端验证 `nvcc --version` 与 `nvidia-smi`。现代 CUDA 不需要旧 `libnvvp` 路径。
...
| 驱动低于 580 分支 | CUDA 13 运行库比驱动系列新 | 升级驱动，或有计划地选择受支持 CUDA 12 栈 |
...
- [ONNX Runtime 1.27 发布](https://github.com/microsoft/onnxruntime/releases/tag/v1.27.0)
...
- [CUDA Toolkit 13.0.2 Python 元数据](https://pypi.org/pypi/cuda-toolkit/13.0.2/json)
- [cuDNN CUDA 13 9.14.0.64 元数据](https://pypi.org/pypi/nvidia-cudnn-cu13/9.14.0.64/json)
