...
> [!TIP]
> **初次阅读？** 可先通过下面的目录了解整份指南，再根据需要前往 A 部分（Windows）、B 部分（Android）或 C 部分（使用自己的模型）。
...
QNN CPU 是参考后端，QNN 2.4 发布包有意不附带该库。如需验证 CPU 后端，请安装匹配版本的 QAIRT，并通过 `--qnn-sdk PATH` 指定其路径。
...
只有目标真机最终显示 `PASS`，才能确认硬件确实参与了执行。这个小模型只用于验证配置和执行路径，不能用于性能评估。

### 完成本教程后

即使初次接触 QNN，也可以按顺序完成以下操作：
...
5. 检查是否发生 CPU 回退，避免把“EP 名称出现在列表中”误判为硬件加速。
6. 调整自己的模型以满足生产要求，再加入 Context Cache 和性能分析。

演示使用确定性的合成小网络，不会下载第三方模型，也不会上传任何用户数据。
...
ONNX Runtime 会优化、划分或编译 ONNX 计算图。QNN EP 将支持的子图转换为 QNN Graph，再交给选定的 QNN 后端执行。**QNN 只有一个 EP，但可以选择多个后端；ONNX Runtime 中并不存在三个不同名称的 QNN EP。**
...
即使电脑搭载 Snapdragon 芯片，**进程架构**也必须匹配。Windows 上的本机 QNN GPU/NPU 推理需要 Windows ARM64 包和原生 ARM64 Python 或应用程序；使用 x64 Python 启动的仍然是 x64 进程。
...
不要单独升级某个 DLL 或 AAR。Backend API、Stub、Skel、固件、插件和 Context Binary 之间都有版本兼容要求。
...
启动脚本会在安装前检查授权状态、ABI、API 和模拟器状态。部分 OEM 设备属性不会明确显示 Qualcomm；此时脚本只会给出警告，不会直接拒绝设备。最终仍以禁用 CPU 回退的 QNN 会话能否成功运行为准。不要执行 `adb pull libcdsprpc.so`，该库必须由 OEM 在设备上管理。
...
首次运行需要下载 Python wheel、Gradle 和体积较大的原生 AAR，可能耗时数分钟；后续运行会复用已经准备好的环境和缓存。使用 `--install` 时，启动脚本会在安装前拒绝模拟器、非 `arm64-v8a` 设备或 API 低于 27 的设备。
...
Android 新手应优先验证 HTP。本次核验使用 Nubia NX711J 真机，设备搭载 Snapdragon 8 Gen 2（`SM8550`、HTP v73）并运行 Android API 35。HTP 在禁用 CPU 回退的条件下多次通过测试；20 次计时运行测得中位延迟为 0.18–0.27 ms，与 ORT CPU 相比的最大误差为 0.0163526。这个小模型不能作为性能基准，延迟波动也说明了这一点。同一设备上的 `libQnnGpu.so` 返回 `QNN_COMMON_ERROR_PLATFORM_NOT_SUPPORTED`，旧版 2.45 Runtime 还会发生 Interface 协商失败。这表示对应 GPU 或运行时组合不受支持，不应通过开启 CPU 回退来规避。Qualcomm 公开的 GPU 文章面向 Snapdragon X Windows Preview，上游 QNN GPU 单元测试也会跳过 ARM64，因此不能仅凭 APK 中存在 `libQnnGpu.so` 就认定手机支持 QNN GPU。
...
部分 Android OEM 系统（包括本次核验使用的 SM8550）会在 `READY` 状态中把 QNN EP 注册设备显示为 `type=CPU`。应用会启用这个 ARM64 注册 Handle，因为不启用时插件不会提供任何可用设备。该信息**不表示计算图在 CPU 上执行**：实际后端由 `backend_type=htp|gpu|cpu` 选择，只有禁用 CPU 回退后的运行结果才能确认执行设备。
...
由于测试图受到完整支持且 ORT CPU 回退已禁用，只要有一个节点必须交给 ORT CPU，会话创建或执行就会失败。结合显式指定的 `backend_type`、成功运行和数值对比，可以确认当前冒烟图确实由目标后端执行；但这不能保证其他模型的算子覆盖率或性能。
...
应用不会硬编码 `htp_arch`，而是由运行时检测设备。与根据产品名称手工选择 Skel 相比，这种方式更可靠。
...
| `session.disable_cpu_ep_fallback` | Session | `1` | 验证完整设备执行时设为 `1`，不允许静默回退 |
...
核验资料时按以下优先级采用信息：带版本的发布说明或源码文档、软件包元数据、Android 平台文档、Qualcomm 工程博客、第三方实践指南。实践资料有助于理解部署选择，但不能取代版本标签中的兼容性声明。
