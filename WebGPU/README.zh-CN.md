...
|---|---|
| 最后验证 | `2026-07-17` |
| 主机 | 取决于浏览器的 Windows、Linux、macOS；原生插件支持范围更窄 |
...
| 验证入口 | `onnxruntime-web-demo/run_demo.bat`、`run_demo.sh` |
| 证明范围 | 独立数学参考、跨 Provider 一致性、严格回退策略，以及可用时的原生 Profile 事件 |

> [!NOTE]
> ORT API/软件包以 ONNX Runtime 与 npm/PyPI 一手资料为准；平台可用性以浏览器/操作系统厂商资料为准。WebGPU，特别是 WebNN，变化很快，发布前必须复查文末实时状态页。

### 发布通道快照

| 组件 | 可安装的最新稳定版 | 本教程 | 上游开发信号 |
|---|---:|---:|---|
| ONNX Runtime Web（npm） | `1.27.0` | `1.27.0` | npm 还发布带日期的 `dev` 构建；本教程不使用它们 |
| ONNX Runtime 核心（PyPI） | `1.27.0` | `1.27.0` | 应等待下一个正式 ORT 标签，不要单独搭配 nightly |
| 原生 WebGPU 插件（PyPI） | `0.1.0` | `0.1.0` | 源码树中的 `VERSION_NUMBER` 已为 `0.3.0`，但它不是公开 PyPI 版本 |

可运行路径有意采用最新的**已发布稳定版**，而不是源码树中能看到的最高版本号。只有兼容的两个原生包都已发布，并通过本仓库严格 Profile/数值对比后，才能升级这一组合。
...
不能只根据这些版本号直接替换固定值。提交新组合前，还要重新检查 npm 导出映射、Python wheel 文件名、插件最低核心要求，并运行严格演示。
...
PyPI 插件 wheel 要求 Python 3.11+，但有意不通过 `Requires-Dist` 硬依赖某个 ONNX Runtime 核心包。官方插件说明要求单独安装兼容核心，把 1.24.4 作为最低版本，并在注册动态库时验证兼容性。本教程固定经过测试的 `onnxruntime==1.27.0` 组合，而不是依赖该最低值。核心包提供 CPython 3.11–3.14 wheel；其 macOS wheel 仅有 arm64，因此即使插件 wheel 本身是 universal2，完整固定原生路径仍只支持 Apple Silicon，不支持 Intel Mac。
...
| 日期 | 检查 | 结果 |
|---|---|---|
| 2026-07-17 | 稳定版仓库、wheel 元数据、已安装 npm 包，以及当前/标签插件源码 | 通过；最新稳定版仍是 ORT Web/核心 1.27.0 与插件 0.1.0。已安装包的导出映射和 WebNN TypeScript 约束与可运行演示一致；更新的插件源码版本号不是已发布 PyPI 版本 |
| 2026-07-16 | 本地浏览器 WASM、ORT Web 1.27.0、COOP/COEP、4 线程 | 通过；使用本地 npm 文件完成精确版本、模型约束和独立数学参考检查 |
| 2026-07-16 | Linux x86-64、Python 3.13.14 直接运行 `launch_demo.py native-webgpu` | 通过；插件发现 NVIDIA 与 Intel 适配器 |
| 2026-07-16 | 在发现到的 NVIDIA 与 Intel 适配器上分别运行原生严格模式 | 两个设备索引都通过；`MatMul`、`Add`、`Relu` 在 `WebGpuExecutionProvider` 上出现性能事件，CPU 节点事件为 0，并通过 CPU 数值对比 |
| 2026-07-16 | VS Code 集成浏览器与独立本地 Chrome 150 配置中的 WebGPU | 两者的预检查都正确报告适配器为空；均不作为浏览器 GPU 成功证据 |
| 2026-07-16 | 当前 Linux Chrome 150 中的 WebNN | `WebMachineLearningNeuralNetwork` 已暴露 `navigator.ml`；随后创建上下文时报告当前本地无头/Linux 配置不支持 WebNN。已验证启动路由、失败诊断、Chromium 功能策略，以及 ORT 1.27 必需的 `{deviceType, context}` 约束，但没有宣称完成 WebNN 硬件运行 |
| 2026-07-16 | Windows 与 macOS 命令 | 已对照官方 wheel 元数据与平台文档；正式结论前仍须在目标硬件执行 |
...
某些 ORT 通用页面仍显示 `onnxruntime-web/experimental`。已发布 1.27.0 包的导出映射包含 `./all`，不包含 `./experimental`；因此 WebNN 专用教程和本演示使用 `onnxruntime-web/all` / `ort.all.min.js`。
...
当前实时 WebNN 教程称提供 `context` 后会忽略其他上下文参数，但已安装 1.27.0 的 TypeScript 声明明确要求这种形式同时提供 `deviceType`。使用固定运行时时应遵循该版本自带的声明；升级时要重新检查此约束。
...
| `--warmups` | `2` | 不计入性能结果的预热次数（仍兼容别名 `--warmup`） |
